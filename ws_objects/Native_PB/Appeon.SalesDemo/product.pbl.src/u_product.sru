...
Select  ProductPhoto.LargePhotoFileName
Into :ls_photoname
...
IF len(ls_photoname) > 0 Then
	...
	SELECT length(ProductPhoto.LargePhoto)
	INTO :ll_data_length
	FROM  Production.ProductPhoto, Production.ProductProductPhoto
	Where (ProductPhoto.ProductPhotoID = ProductProductPhoto.ProductPhotoID) 
	And (ProductProductPhoto.ProductID = :proid)
...
Choose Case iuo_currentdw.ClassName()
	Case "dw_browser"		
		...
		if tab_1.tabpage_1.dw_productlist.rowcount() > 0 then
			messagebox('Information',"This sub category didn't delete, because it is exist product data")
			return -1
		end if
...
//		end if
		...
		IF li_ret = 1 Then
			...
			DELETE FROM production.productcosthistory  WHERE    production.productcosthistory.productid = :ll_productid;
			IF Sqlca.Sqlcode <> 0 THEN
				RollBack;
				Return -1
			END IF
			
			DELETE FROM production.productinventory  WHERE    production.productinventory.productid = :ll_productid;
			IF Sqlca.Sqlcode <> 0 THEN
				RollBack;
				Return -1
			END IF
			...
			delete from production.billofmaterials where production.billofmaterials.componentid = :ll_productid or production.billofmaterials.productassemblyid = :ll_productid ;
			if sqlca.sqlcode <> 0 then
				rollback;
				return -1
			end if
			...
			select count(*) into :ll_count from production.workorder where productid = :ll_productid;
			if ll_count> 0 then
				messagebox('Information',"This product cann't delete, because it has workorder data exist")
				return -1
			end if
			...
			IF iuo_currentdw.ClassName() = "dw_master" Then
				...
				IF tab_1.tabpage_1.dw_productlist.Update() <> 1 THEN
					RollBack;
					Return -1
				END IF
...
IF ls_dwname =  "dw_master" and ldws_sign = NewModified! Then
	...
	SELECT Max(productid) INTO :ll_pkid FROM production.product;
	IF isnull(ll_pkid) THEN
		ll_pkid=0
	END IF 
...
//	iuo_currentdw.Filter()
	...
	iuo_currentdw.setitem(1,"finishedgoodsflag",'0')
End IF
...
SELECT Max(ProductPhotoID) + 1 INTO :ll_pk FROM Production.ProductPhoto; 
if isnull(ll_pk) or ll_pk = 0 then ll_pk = 1
ldt_now = DateTime(Today(), Now())
