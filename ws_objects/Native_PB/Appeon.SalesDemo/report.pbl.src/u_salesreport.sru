...
//ldt_from = DateTime(String (ll_year - 1) + Mid(ls_from ,5))
//ldt_to = DateTime(String (ll_year - 1) + Mid(ls_to ,5))
...
lds_data.SetTransObject(Sqlca)
lds_data.Retrieve(ll_categoryid,  string(ll_monthfrom), string(ll_monthto))
...
lds_data.SetTransObject(Sqlca)
lds_data.Retrieve(ll_subcategoryid,  string(ll_monthfrom), string(ll_monthto))
...
tab_1.tabpage_2.dw_master.GetChild("dw_productsalesreport", ldwc_prod)
ldwc_prod.SetTransObject(Sqlca)
...
ldwc_prod.Retrieve(ll_subcategoryid, ldt_from, ldt_to)
