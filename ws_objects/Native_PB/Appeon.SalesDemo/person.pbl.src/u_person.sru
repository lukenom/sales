...
Choose case iuo_currentdw.ClassName()
	Case	"dw_browser", "dw_persondetail"		
		...
		IF li_ret = 1 Then
			...
			select sales.customer.customerid into :ll_customerid from sales.customer where sales.customer.personid = :ll_personid;

			select count(*) into :ll_order from sales.salesorderheader where sales.salesorderheader.customerid = :ll_customerid;
			if ll_order > 0 then
				messagebox('Information',"This customer didn't delete, because this customer exist salesorder")
				return -1
			end if
	...
	Case "dw_cust"	
		...
		IF li_ret = 1 Then			
			...
			delete from sales.salesorderheader wehre sales.salesorderheader.customerid = :ll_customerid;
			if sqlca.sqlcode <> 0 then
				rollback;
				return -1
			end if
...
If tab_1.tabpage_1.dw_persondetail.Modifiedcount() > 0 Then
	...
	IF ldws_status = Newmodified! Then
		...
		SELECT Max(BusinessEntityID) INTO :ll_pk FROM Person.BusinessEntity;
		IF isnull(ll_pk) THEN
			ll_pk=0
		END IF 
